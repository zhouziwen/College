...
public class EaseSmileUtils {
	...
	static {
	    EaseEmoIcon[] emojicons = EaseDefaultEmojiconDatas.getData();
	    for(int i = 0; i < emojicons.length; i++){
	        addPattern(emojicons[i].getEmojiText(), emojicons[i].getIcon());
	    }
	    EaseUI.EaseEmoIconInfoProvider emojiconInfoProvider = EaseUI.getInstance().getEmoIconInfoProvider();
	    if(emojiconInfoProvider != null && emojiconInfoProvider.getTextEmoIconMapping() != null){
	        for(Entry<String, Object> entry : emojiconInfoProvider.getTextEmoIconMapping().entrySet()){
