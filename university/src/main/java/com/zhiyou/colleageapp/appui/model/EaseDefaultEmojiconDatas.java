...
public class EaseDefaultEmojiconDatas {
    ...
    private static final EaseEmoIcon[] DATA = createData();
    
    private static EaseEmoIcon[] createData(){
        EaseEmoIcon[] datas = new EaseEmoIcon[icons.length];
        for(int i = 0; i < icons.length; i++){
            datas[i] = new EaseEmoIcon(icons[i], emojis[i], EaseEmoIcon.Type.NORMAL);
        }
        return datas;
    }
    
    public static EaseEmoIcon[] getData(){
        return DATA;
    }
}
