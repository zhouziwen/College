...
import com.zhiyou.colleageapp.appui.model.EaseDefaultEmojiconDatas;
...
import com.zhiyou.colleageapp.appui.widget.emojicon.EaseEmojiconMenuBase;
...
public class EaseChatInputMenu extends LinearLayout {
    ...
    /**
     * initEase view 此方法需放在registerExtendMenuItem后面及setCustomEmojiconMenu，
     * setCustomPrimaryMenu(如果需要自定义这两个menu)后面
     * @param emojiconGroupList 表情组类别，传null使用easeui默认的表情
     */
    public void init(List<EaseEmoIconGroupEntity> emojiconGroupList) {
        if(inited){
            return;
        }
        // 主按钮菜单栏,没有自定义的用默认的
        ...
        if(emojiconMenu == null){
            ...
            if(emojiconGroupList == null){
                emojiconGroupList = new ArrayList<EaseEmoIconGroupEntity>();
                emojiconGroupList.add(new EaseEmoIconGroupEntity(R.drawable.ee_1,  Arrays.asList(EaseDefaultEmojiconDatas.getData())));
            }
    ...
    protected void processChatMenu() {
        ...
        emojiconMenu.setEmojiconMenuListener(new EaseEmojiconMenuListener() {
            ...
            @Override
            public void onExpressionClicked(EaseEmoIcon emojicon) {
                if(emojicon.getType() != EaseEmoIcon.Type.BIG_EXPRESSION){
